...
using Rimot.Server.Components;
using Rimot.Server.Components.ModalContents;
...
namespace Rimot.Server.Pages
{
    public partial class ManageOrganization : AuthComponentBase
    {
        ...
        private readonly List<RimotUser> _orgUsers = new();
        ...
        [Inject]
        private UserManager<RimotUser> UserManager { get; set; }
        ...
        private async Task DeleteUser(RimotUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task EditDeviceGroups(RimotUser user)
        {
        ...
        private async Task ResetPassword(RimotUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task SendInvite()
        {
            ...
            else
            {
                ...
                var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser, "Invitation to Organization in Rimot",
                        $@"<img src='{NavManager.BaseUri}images/Rimot_Logo.png'/>
                            <br><br>
                            Hello!
                            <br><br>
        ...
        private void SetUserIsAdmin(ChangeEventArgs args, RimotUser orgUser)
        {
            if (!User.IsAdministrator)
            {
                return;
