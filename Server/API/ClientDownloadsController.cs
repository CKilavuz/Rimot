        ...
        public async Task<IActionResult> GetDesktop(string platformID)
        {
            switch (platformID)
            {
                case "WindowsDesktop-x64":
                    {
                        var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "Win-x64", "Rimot_Desktop.exe");
                        return await GetDesktopFile(filePath);
                    }
                case "WindowsDesktop-x86":
                    {
                        var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "Win-x86", "Rimot_Desktop.exe");
                        return await GetDesktopFile(filePath);
                    }
                case "UbuntuDesktop":
                    {
                        var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "Linux-x64", "Rimot_Desktop");
                        return await GetDesktopFile(filePath);
                    }
                case "MacOS-x64":
                    {
                        var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "MacOS-x64", "Rimot_Desktop");
                        return await GetDesktopFile(filePath);
                    }
                case "MacOS-arm64":
                    {
                        var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "MacOS-arm64", "Rimot_Desktop");
                        return await GetDesktopFile(filePath);
                    }
                default:
                    return NotFound();
                ...
                if (await _fileLock.WaitAsync(TimeSpan.FromSeconds(15)))
                {
                    switch (platformID)
                    {
                        case "WindowsInstaller":
                            {
                                var filePath = Path.Combine(_hostEnv.WebRootPath, "Content", "Rimot_Installer.exe");
                                var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read);
                                var organization = _dataService.GetOrganizationById(organizationId);
                                var relayCode = organization.RelayCode;
                                return File(fs, "application/octet-stream", $"Rimot_Install-[{relayCode}].exe");
                            }
