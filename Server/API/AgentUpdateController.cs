    ...
    public class AgentUpdateController : ControllerBase
    {
        ...
        public async Task<ActionResult> DownloadPackage(string platform, string downloadId)
        {
            try
            {
                ...
                {
                    case "win-x64":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Rimot-Win10-x64.zip");
                        break;
                    case "win-x86":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Rimot-Win10-x86.zip");
                        break;
                    case "linux":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Rimot-Linux.zip");
                        break;
                    case "macos-x64":
                        filePath = Path.Combine(HostEnv.WebRootPath, "Content", "Rimot-MacOS-x64.zip");
                        break;
                    default:
                ...
                return File(fileStream, "application/octet-stream", "RimotUpdate.zip");
            }
            catch (Exception ex)
            {
                _downloadingAgents.Remove(downloadId);
        ...
        private async Task<bool> CheckForDeviceBan(string deviceIp)
        {
            ...
            if (AppConfig.BannedDevices.Contains(deviceIp))
            {
                ...
                foreach (var bannedDevice in bannedDevices)
                {
                    ...
                    var command = "sc delete Rimot_Service & taskkill /im Rimot_Agent.exe /f";
