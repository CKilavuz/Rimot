    ...
    public class UpgradeService : IUpgradeService
    {
        ...
        public async Task<bool> IsNewVersionAvailable()
        {
            ...
            try
            {
                ...
                var response = await client.GetAsync("https://github.com/rimot-support/Rimot/releases/latest");
                var versionString = response.RequestMessage.RequestUri.ToString().Split("/").Last()[1..];
                var remoteVersion = Version.Parse(versionString);
                var filePath = Directory.GetFiles(Directory.GetCurrentDirectory(), "Rimot_Server.dll", SearchOption.AllDirectories).First();
