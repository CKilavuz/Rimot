...
param (
	...
	[string]$HostName,
     # The name to use for the IIS Application Pool for the Rimot site.
    [Parameter(Mandatory=$True)]
	[string]$AppPoolName,
     # The name to use for the IIS site.
    [Parameter(Mandatory=$True)]
	[string]$SiteName,
    # The folder path where the Rimot server files are located.
    [Parameter(Mandatory=$True)]
    [string]$SitePath,
...
$Host.UI.RawUI.WindowTitle = "Rimot Setup"
Clear-Host
...
if ($OS.Name.ToLower().Contains("home") -or $OS.Caption.ToLower().Contains("home")) {
    Wrap-Host
    Wrap-Host "Error: Windows Home version does not have the necessary features to run Rimot." -ForegroundColor Red
    Do-Pause
    return
}
...
Wrap-Host
Wrap-Host "**********************************"
Wrap-Host "         Rimot Setup" -ForegroundColor Cyan
Wrap-Host "**********************************"
Wrap-Host
Wrap-Host "Hello, and thank you for trying out Rimot!" -ForegroundColor Green
Wrap-Host
Wrap-Host "This setup script will create an IIS site and install Rimot on this machine." -ForegroundColor Green
Wrap-Host
Do-Pause
Clear-Host
