...
namespace Server.Installer
{
    ...
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            {
                ConsoleHelper.WriteError("Rimot Server can only be installed on Linux or Windows.");
                return;
            }
            ...
            ConsoleHelper.WriteLine("Thank you for trying Rimot!  This installer will guide you " +
                "through deploying the Rimot server onto this machine.");
            ...
            ConsoleHelper.WriteLine("If using GitHub Actions, you will need to enter a GitHub Personal Access Token, " +
                "which will allow this app to access your fork of the Rimot repo.  You can generate a PAT at " +
            ...
            ConsoleHelper.WriteLine("If you haven't already, please go to the Actions tab in your Rimot repo " +
            ...
            {
                cliParams.InstallDirectory = ConsoleHelper.ReadLine("Which directory should the server files be extracted to (e.g. /var/www/rimot/)?").Trim();
            }

            while (cliParams.ServerUrl is null)
            {
                var url = ConsoleHelper.ReadLine("What is your server's public URL (e.g. https://rimot.support)?").Trim();
                        ...
                        case "-s":
                            {
                                ...
                                ConsoleHelper.WriteError("--server-url parameter is invalid.  Must be a valid URL (e.g. https://rimot.support).");
                                return false;
                            }
        ...
        {
            ConsoleHelper.WriteLine("Rimot Server Installer", 0, ConsoleColor.Cyan);
            ConsoleHelper.WriteLine("Builds a customized Rimot server using GitHub actions " +
            ...
            ConsoleHelper.WriteLine("\t--github-username, -u    Your GitHub username, where the forked Rimot repo exists.", 1);
            ...
            ConsoleHelper.WriteLine("\t--server-url, -s    The public URL where your Rimot server will be accessed (e.g. https://rimot.support).", 1);
            ...
            ConsoleHelper.WriteLine("\t--web-server, -w    Number.  The web server that will be used as a reverse proxy to forward " +
                "requests to the Rimot server.  Select the appropriate option for your operating system and web server.  " +
            ...
            ConsoleHelper.WriteLine("sudo ./Rimot_Server_Installer -b false -u rimot-support -p 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 -s https://rimot.support -i /var/www/rimot/ -r latest -c true -w 0", 1);

            ConsoleHelper.WriteLine("Example (use pre-built package):");
            ConsoleHelper.WriteLine("sudo ./Rimot_Server_Installer -b true -s https://rimot.support -i /var/www/rimot/ -w 0");
        }
    }
}
