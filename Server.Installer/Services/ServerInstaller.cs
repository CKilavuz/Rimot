...
namespace Server.Installer.Services
{
    ...
    public class ServerInstaller : IServerInstaller
    {
        ...
        {
            var zipPath = Path.Combine(Path.GetTempPath(), "Rimot_Server.zip");
            ...
            if (cliParams.UsePrebuiltPackage == true)
            {
                ...
                var releaseFile = cliParams.WebServer == WebServerType.IisWindows ?
                    "https://github.com/rimot-support/Rimot/releases/latest/download/Rimot_Server_Win-x64.zip" :
                    "https://github.com/rimot-support/Rimot/releases/latest/download/Rimot_Server_Linux-x64.zip";
                ...
                if (cliParams.CreateNew == true)
                {
                    ...
                    if (!dispatchResult)
                    {
                        ConsoleHelper.WriteError("GitHub API call to trigger build action failed.  Do you have " +
                            "Actions enabled on your forked Rimot repo on the Actions tab?  If not, enable them and try again. " +
            ...
            if (cliParams.WebServer == WebServerType.IisWindows)
            {
                ...
                {
                    Process.Start("powershell.exe", "-Command & \"{ Stop-WebAppPool -Name Rimot -ErrorAction SilentlyContinue }\"").WaitForExit();
                    Process.Start("powershell.exe", "-Command & \"{ Stop-Website -Name Rimot -ErrorAction SilentlyContinue }\"").WaitForExit();
        ...
        private async Task LaunchExternalInstaller(CliParams cliParams)
        {
            ...
            var resourcesPath = "Rimot.Server.Installer.Resources.";
            ...
            if (cliParams.WebServer.Value == WebServerType.IisWindows)
            {
                ...
                {
                    Arguments = $"-f \"{filePath}\" -AppPoolName Rimot -SiteName Rimot " +
